...
def evaluate(dataset_type: str, k: int = 3) -> None:
    ...
    dataset_path = (
        ...
        f"dataset_{dataset_type}_public.json"
    )

    if not os.path.exists(dataset_path):
        log_message(
            "Skipping auto-evaluate for "
            f"{dataset_type}: '{dataset_path}' not found.",
            LogType.WARNING,
        )
        return
    ...
    with open(dataset_path, "r", encoding="utf-8") as f:
        data = json.load(f)
        question_list = [
            AnsweredQuestion(**d) for d in data.get("rag_questions", [])
        ]
    ...
    for question in tqdm(question_list, desc=f"Evaluating {dataset_type}"):
        ...
        for source in question.sources:
            gt_path = (
                source.file_path
                if source.file_path.startswith("./")
                else f"./{source.file_path}"
            )
            ...
            for found in found_files:
                pred_path = (
                    found.file_path
                    if found.file_path.startswith("./")
                    else f"./{found.file_path}"
                )
                ...
                overlap = max(
                    0,
                    min(gt_end, found.last_character_index)
                    - max(gt_start, found.first_character_index),
                )
    ...
    log_message(
        f"RAG evaluation:\nRecall@{k} ({dataset_type}) = {final_recall:.2f}%",
        LogType.INFO,
    )

    if (dataset_type == "code" and final_recall < 45.0) or (
        dataset_type == "docs" and final_recall < 55.0
    ):
        log_message(
            f"Recall@{k} for {dataset_type} has not been achieved!",
            LogType.WARNING,
        )


def evaluate_student_search_results(
    student_answer_path: str, dataset_path: str, k: int
) -> None:
    """Evaluate saved student search results against a ground-truth dataset."""
    if not os.path.exists(student_answer_path):
        raise LLMException(
            f"Student answer file not found: {student_answer_path}"
        )
    if not os.path.exists(dataset_path):
        raise LLMException(
            f"Ground-truth dataset file not found: {dataset_path}"
        )
    ...
    with open(student_answer_path, "r", encoding="utf-8") as f:
        ...
        try:
            ...
            raise LLMException(
                f"Invalid student answers format. Error: {e}"
            )
    ...
    if not recalls:
        log_message(
            "No matching questions found between the student "
