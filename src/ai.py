...
class AI:
    ...
    def _ensure_server_running(self) -> None:
        ...
        if not is_ollama_alive():
            log_message(
                "Ollama server not running. Launching...",
                LogType.WARNING,
            )
            try:
                subprocess.Popen(
    ...
    def RAG(
        self, query: str, k: int = 3, max_length: int = 1024
    ) -> tuple[str, float]:
        ...
        examples: list[tuple[str, str]] = [
            (
                (
                    "Where can I find information about using "
                    "generative models in vLLM?"
                ),
                "Information about using generative models can be found "
                "on the generative models page as referenced in the "
                "supported models documentation.",
            ),
            (
                (
                    "What conditions must be met for ModelRunner "
                    "to use CUDA graphs?"
                ),
                "Two conditions must be met. First, prefill_meta must "
                "be None. Second, decode_meta.use_cuda_graph must be "
                "True. When both are satisfied, the ModelRunner uses "
                "the virtual engine graph runners instead of the "
                "regular model.",
            ),
        ]
        ...
        try:
            ...
            load_duration: float = float(
                getattr(
                    response,
                    "load_duration",
                    response.get("load_duration", 0),
                )
            )
